...
def privacy_metrics(real,fake,data_percent=15):
  ...
  DNS = metrics.pairwise_distances(df_real_scaled, Y=df_fake_scaled, metric='minkowski', n_jobs=-1)


  # Computing first and second smallest nearest neighbour distances between real and synthetic
  smallest_two_indexes_rf = [DNS[i].argsort()[:2] for i in range(len(DNS))]
  smallest_two_rf = [DNS[i][smallest_two_indexes_rf[i]] for i in range(len(DNS))]       

  # Computing 5th percentiles for DCR and NNDR between and within real and synthetic datasets
  min_DNS = np.array([i[0] for i in smallest_two_rf])
  DNS = np.percentile(min_DNS,5)

  nn_ratio_rf = np.array([i[0]/i[1] for i in smallest_two_rf])
  NNDR = np.percentile(nn_ratio_rf,5)
